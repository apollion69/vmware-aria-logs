...
class TestQueryEventsTool:
    ...
    def test_returns_json_events(self, mock_auth: MagicMock, mock_query: MagicMock) -> None:
        ...
        result = json.loads(query_events(lookback_minutes=30, search_term="error", limit=10))
    ...
    @patch("vmware_aria_logs.clients.loginsight.LogInsightClient.query_events")
    @patch("vmware_aria_logs.clients.loginsight.LogInsightClient.authenticate")
    def test_deduplicates_events(self, mock_auth: MagicMock, mock_query: MagicMock) -> None:
        mock_auth.return_value = "fake-token"
        mock_query.return_value = [
...
class TestDetectIncidentsTool:
    ...
    @patch("vmware_aria_logs.clients.loginsight.LogInsightClient.query_events")
    @patch("vmware_aria_logs.clients.loginsight.LogInsightClient.authenticate")
    def test_returns_incidents(self, mock_auth: MagicMock, mock_query: MagicMock) -> None:
        mock_auth.return_value = "fake-token"
        mock_query.return_value = [
...
class TestGetVersionTool:
    ...
    @patch("vmware_aria_logs.clients.loginsight.LogInsightClient.authenticate")
    def test_returns_version_info(self, mock_auth: MagicMock, mock_probe: MagicMock) -> None:
        mock_auth.return_value = "fake-token"
...
class TestFindVropsResourcesTool:
    ...
    def test_returns_resources(self, mock_auth: MagicMock, mock_find: MagicMock) -> None:
        mock_auth.return_value = "fake-token"
        mock_find.return_value = [
            {
                "identifier": "res-123",
                "resourceKey": {"name": "vm-web01", "resourceKindKey": "VirtualMachine", "adapterKindKey": "VMWARE"},
    ...
    def test_returns_error_without_vrops_config(self, monkeypatch: pytest.MonkeyPatch) -> None:
...
class TestListDashboardsTool:
    ...
    @patch("vmware_aria_logs.clients.loginsight.LogInsightClient.authenticate")
    def test_returns_dashboards(self, mock_auth: MagicMock, mock_dash: MagicMock) -> None:
        mock_auth.return_value = "fake-token"
    ...
    @patch("vmware_aria_logs.clients.loginsight.LogInsightClient.list_dashboards")
    @patch("vmware_aria_logs.clients.loginsight.LogInsightClient.authenticate")
    def test_empty_returns_message(self, mock_auth: MagicMock, mock_dash: MagicMock) -> None:
        mock_auth.return_value = "fake-token"
        mock_dash.return_value = []
        from vmware_aria_logs.server import list_dashboards
        result = json.loads(list_dashboards())
...
class TestGetVropsAlertsTool:
    ...
    def test_no_vrops_config_returns_error(self, monkeypatch: pytest.MonkeyPatch) -> None:
        monkeypatch.setenv("LI_BASE_URL", "https://li.test")
        monkeypatch.setenv("LI_USERNAME", "admin")
        monkeypatch.setenv("LI_PASSWORD", "pw")
        monkeypatch.delenv("VROPS_BASE_URL", raising=False)
...
class TestNoConfigError:
    def test_query_events_fails_without_base_url(self, monkeypatch: pytest.MonkeyPatch) -> None:
